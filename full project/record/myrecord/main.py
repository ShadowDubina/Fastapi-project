...
async def get_records(request:Request, offset:int=None, limit:int=None)->List[models.Record]:
    async with request.app.state.session_maker() as session:
        records = await crud.get_records(session, offset=offset, limit=limit)
        return records